...
class SERVER:
        ...
        with zipfile.ZipFile(_path, "r") as zip:
            ...
            if self.cc == "en":
                ...
                sort = sorted(
                    tsvs,
                    key=lambda x: (
                        region.index(x.split("_")[0].split("download")[1]),
    ...
    def get_zip_link(self, index: int):
        ...
        if version < 1000000:
            ...
            version = "{}_{:06d}_{:02d}_{:02d}".format(
                cc, version // 100, index, version % 100
            )
...
class ITEM:
    ...
    def parse_data(self, folder: str, list: str, data: bytes):
        for _line in list.splitlines()[1:]:
            ...
            if not os.path.exists(os.path.join(_path, name)) or self.get_hash(
