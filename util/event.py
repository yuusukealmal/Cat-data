...
class EventNew:
    ...
    def getNyankoSignature(self, json_text):
        ...
        return (
            random_data
            + self.hmacSha256(
                bytes(self.accountCode + random_data, "utf-8"), json_text
            ).hex()
        )
    ...
    def generateAccount(self):
        ...
        passwordResponse = self.getPostResponse(
            self.passwordLink, json.dumps(passwordHeaderData).replace(" ", "")
        )
        if passwordResponse.json()["statusCode"] == 1:
            self.password = passwordResponse.json()["payload"]["password"]
            self.passwordRefreshToken = passwordResponse.json()["payload"][
    ...
    def generateJWTToken(self):
        ...
        tokenResponse = self.getPostResponse(
            self.jwtLink, json.dumps(tokenData).replace(" ", "")
        )
