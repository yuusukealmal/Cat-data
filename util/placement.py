...
def webhook(cc: str, res: dict):
    ...
    for event in res:
        ...
        if 200 <= result.status_code < 300:
            ...
            print(
                f"{t_str} Not sent with {result.status_code}, response:\n{result.json()}"
            )
...
def parse(cc: str, res: dict, notify: bool):
    ...
    for event in res["notice"]["data"]:
        ...
        if r.status_code != 200:
            continue
        if ((t := convertUnix(event["start"])) > int(time.time())) and (
            event["id"] not in j[cc.upper()]["uuid"]
        ):
            dic.append(
                {
                    "uuid": event["id"],
            ...
            with open(
                os.path.join(
                    os.getcwd(), "Data", "placement", cc.upper(), f"{event['id']}.png"
                ),
                "wb",
            ) as f:
                f.write(r.content)
