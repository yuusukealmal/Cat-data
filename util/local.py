...
class ITEM(APK):
    ...
    def get_aes(self):
        if self.way == "new":
            _aes = AES.new(
                bytes.fromhex(self.PACK_KEY), AES.MODE_CBC, bytes.fromhex(self.IV_KEY)
            )
        else:
    ...
    def get_folder(self):
        ...
        target_dir = os.path.join(
            base_dir, "Data", "local", self.package, self.item.split("/")[-1]
        )
    ...
    def parse(self):
        for _line in self.list.splitlines()[1:]:
            ...
            if (not os.path.exists(_path)) or (
                self.get_hash(_path) != self.get_hash(_data)
            ):
                if (
                    self.cc == "jp"
                    and any([i in name for i in ["imgcut", "mamodel", "maanim"]])
                    and self.item == "NumberLocal"
                ):
...
def parse_version_str(version: int):
    ...
    return ".".join(
        version_str[i : i + 2].lstrip("0") or "0" for i in range(0, len(version_str), 2)
    )
...
def local(way: str, apk=None, xapk=None, remote=False):
    if way != "latest":
        ...
        for i in _process:
            ...
            git_push("add", f"[ci] Update Certain Game {i.upper()} Local Files")
            os.remove(xapk)
            if remote:
                server(xapk=_path)
                git_push("add", f"[ci] Update Certain Game {i.upper()} Server Files")
            try:
                os.remove(_path)
            except:
                pass
